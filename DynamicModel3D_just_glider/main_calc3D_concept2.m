...
record_flag = true
if record_flag
    vid = VideoWriter(['SimulRec3D_', date, '_ID', num2str(randi(10000)), '.avi']);
    vid.FrameRate = 20;
    open(vid);
end
...
for n = 2:Ncalc
    ...
    joy_roll = axis(joy,1);
    phiwing = 2*pi/3 * 0.5*(1-axis(joy,4));
    phiaill = joy_roll*phiailmax;
    phiailr = -phiaill;
...
%     Pcons_rarr(n) = Pcons_r;
    ...
    if ((n-1)/Mdraw - round((n-1)/Mdraw)) == 0
        VisualizeUAV3D(Rn, psi, theta, gamma, phiwing, phiaill, phiailr, phielev, visarrstruct, hax, Lcube);
...
%         plot3(zrefarr, xrefarr, yrefarr, 'm*')
        ...
        %plot3(Rn(3) + [0 Vn(3)], Rn(1) + [0 Vn(1)], Rn(2) + [0 Vn(2)], 'y', 'linewidth', 2); 
        hold off
