function [] = VisualizeUAV3D(Ruav, psi, theta, gamma, phi_tilt, ail_defl, elev_defl, visarrstruct, hax, Lcube)
...
altitude_coef = min(1, exp(-0.1*(Ruav(2)-Lcube/2)));
dXZground = Lcube/8;
...
Xground1 = [xgroundticks; xgroundticks]; Zground1 = Ruav(3) + repmat(altitude_coef*Lcube/2*[-1; 1], 1, length(xgroundticks)); Yground1 = max(zeros(size(Xground1)), Ruav(2)-Lcube/2);
Zground2 = [zgroundticks; zgroundticks]; Xground2 = Ruav(1) + repmat(altitude_coef*Lcube/2*[-1; 1], 1, length(zgroundticks)); Yground2 = max(zeros(size(Xground2)), Ruav(2)-Lcube/2);
...
Xail00right = [0, -l1*cos(ail_defl); 0, -l2*cos(ail_defl)] + Broot/5 - Bail;
Yail00right = [0, l1*sin(ail_defl); 0, l2*sin(ail_defl)];
...
axis([Ruav(3) + Lcube/2*hor_coef*[-1, 1], Ruav(1) + Lcube/2*hor_coef*[-1, 1], Ruav(2) + Lcube/2*[-1, 1]])
