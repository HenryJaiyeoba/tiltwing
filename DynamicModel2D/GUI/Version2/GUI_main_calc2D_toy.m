...
if ~pauseflag && ~simulateflag
    ...
    record_flag = true
    if record_flag
        vid = VideoWriter(['SimulRec2D_', date, '_ID', num2str(randi(10000)), '.avi']);
        vid.FrameRate = 30;
        open(vid);
    end
...
%     Tback2hover = 30;
    ...
    %main motors
    Tstatic = 1.3 * m_UAV*g_grav; %maximal static thrust
    ...
    Lhalf_trajgr = 2; %box halfsize for trate pitch controller to provide the desired hor. speed (Vx=0, basically)
    ...
    for n = 2:Ncalc
        ...
        % UAV dynamics
        %calcualate thrust
        throttle = 0.5*(1-axis(joy,4));
        thetaset =  thetaset0 + thetasetmax * -axis(joy,3);
        button1flag =  button(joy,1);
        button2flag =  button(joy,2);
...
%                         
            ...
            axis(Lhalf_trajgr*[-1.5, 1.5, -1, 1] + [Rrefgr(1), Rrefgr(1), Rrefgr(2), Rrefgr(2)]);
            if max(abs(Rn - Rrefgr)) >= (0.8*Lhalf_trajgr)
                Rrefgr = Rn + 0.7*(Rn - Rrefgr);
              
            end
            
            %to display time and speed properly
            curtime = now;
            %convert current time into h-m-s-frac
            t_sec = (curtime - floor(curtime)) * 24*3600;
            t_h = floor(t_sec/3600);
            t_m = floor((t_sec - t_h*3600)/60);
            t_s = t_sec - t_h*3600 - t_m*60;
            V_disp = 3.6*norm(Vn);
            V_disp = num2str(V_disp, '%.1f');

            title(['Time = ' num2str(t_h) ':' num2str(t_m) ':' num2str(t_s, '%0.2f') '; UAV speed = ' V_disp ' km/h'])
